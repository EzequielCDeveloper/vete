...
exports.getById = async (req, res) => {
  try {
    ...
    json(res, {
      ...procs[0],
      id: String(procs[0].id),
      precio: Number(procs[0].precio),
      active: Boolean(procs[0].active),
    });
  } catch (err) { serverError(res, err); }
};
...
exports.create = async (req, res) => {
  try {
    ...
    json(res, { id: String(result?.id || '0'), nombre, descripcion, precio: Number(precio), active: true, createdBy: req.user.username }, 201);
  } catch (err) {
